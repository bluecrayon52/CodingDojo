...
def new_message():
	...
	if len(request.form['content']) < 5:
		flash('Message must be at least 5 characters long', 'content')
		flash('is-invalid', 'content_valid')
		is_valid = False

	else: 
		flash('is-valid', 'content_valid')

	flash(request.form['content'], 'content_value')
	
	# validate recipient 
	if len(request.form['receiver']) < 1:
		flash('Please select a recipient', 'receiver')
		flash('is-invalid', 'receiver_valid')
		is_valid = False
